...
```java
{
  "peerId": "abc123...",       // 本机唯一特征码（SHA-256 of 硬件信息）
  "name": "我的电脑",           // 用户自定义名称
  "addresses": ["192.168.1.5", "fe80::1"],  // 本机 IP 列表
  "port": 19527,              // P2P 服务端口
  "fingerprint": "SHA256:..." // TLS 证书指纹
}
```
...
① 查本地 p2p_agent_authorization 表
   └─ WHERE peer_id=? AND agent_config_id=? AND direction='sent' AND status='active'
...
| 序列化 | JSON（非 Protobuf） | P2P 消息量小，JSON 调试友好，Protobuf 依赖虽在但未强制使用 |
