...
**CodeCraft** 是一个基于 AI Agent 的桌面端智能编程助手。用户通过聊天界面给 AI 下达编程任务，AI 自动调用 30+ 种工具（读写文件、执行命令、操作 Git、搜索网络等）完成任务，支持多 Agent 并行协作。
...
│  │  │    ├─ SkillMatcher (技能匹配)               │              │  │
│  │  │    └─ SnapshotService (快照备份/回滚)        │              │  │
│  │  │                                            │              │  │
│  │  │  ★ P2pAgentService (P2P远程协作)            │              │  │
│  │  │    ├─ P2pConnectionManager (连接池管理)      │              │  │
│  │  │    ├─ P2pServer / P2pClient (Netty网络)     │              │  │
│  │  │    └─ QrCodeSignaling (二维码信令交换)       │              │  │
│  │  └─────────────────────┬──────────────────────┘              │  │
│  │                        │                                      │  │
│  │  ┌─────────────────────┴──────────────────────┐              │  │
│  │  │          Tool 工具系统 (30+ 工具)            │              │  │
│  │  │  ┌────────────┐ ┌──────────┐ ┌──────────┐ │              │  │
│  │  │  │ ToolRegistry│ │ToolExecutor│ │Permission│ │              │  │
│  │  │  │ (注册中心)  │ │ (执行引擎)│ │Pipeline  │ │              │  │
│  │  │  └────────────┘ └──────────┘ └──────────┘ │              │  │
│  │  │  ┌──────────────────────────────────────┐  │              │  │
│  │  │  │ impl/: 30+ 工具实现                   │  │              │  │
│  │  │  │ read/write/edit/delete/grep/glob     │  │              │  │
│  │  │  │ git_*/run_*/web_*/fork/collect/...   │  │              │  │
│  │  │  └──────────────────────────────────────┘  │              │  │
│  │  │  ┌──────────────────────────────────────┐  │              │  │
│  │  │  │ postedit/: 后处理管线                 │  │              │  │
│  │  │  │ Formatter → Diagnostic → 结果合并     │  │              │  │
│  │  │  └──────────────────────────────────────┘  │              │  │
│  │  └─────────────────────┬──────────────────────┘              │  │
│  │                        │                                      │  │
│  │  ┌─────────────────────┴──────────────────────┐              │  │
│  │  │        数据层                               │              │  │
│  │  │  MyBatis Mapper ←→ H2 嵌入式数据库          │              │  │
│  │  │  Caffeine 本地缓存 (替代 Redis)             │              │  │
│  │  │  Milvus 向量数据库 (可选, 语义检索)          │              │  │
│  │  └────────────────────────────────────────────┘              │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                    │
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │              外部依赖                                        │  │
│  │  ┌──────────┐  ┌───────────┐  ┌───────────────────────────┐ │  │
│  │  │ DeepSeek │  │  Milvus   │  │ GitHub API (Git 操作远程)  │ │  │
│  │  │ API      │  │ (向量库)  │  │ Bing Search (web_search)   │ │  │
│  │  └──────────┘  └───────────┘  └───────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
```
...
│                                                                  │
│  ⑤ 发送 HTTP 请求 → DeepSeek API（WebFlux + SSE 流式读取）       │
│                                                                  │
│  ⑥ AI 返回后 → 如果有 tool_calls → ToolLoopManager 接管         │
│     ┌──────────────────────────────────────────────────────┐    │
...
│     └──────────────────────────────────────────────────────┘    │
│                                                                  │
│  ⑦ 返回最终 AI 响应 + 工具执行摘要 → 前端 SSE 流                 │
│                                                                  │
│  ⑧ 后台异步：                                                   │
│     ├─ 消息持久化（MessagePersister）                            │
│     ├─ 异步预压缩（CompactionService.asyncPrecompress）          │
│     └─ 快照创建（首次文件修改前 SnapshotService.create）         │
└──────────────────────────────────────────────────────────────────┘
```
...
主 Agent (运行在用户会话中)
    │
    │ fork_agent("sub-1", "写 UserService", [read_file,write_file,...])
    ▼
┌─ AgentForkManager.forkAgent() ────────────────────────────────────┐
│  ① 在 sub_agent_log 表创建记录（status=running）                    │
...
| **P2P 网络** | Netty + Protobuf | 高性能二进制协议，适合点对点实时通信 |
...
│   │   ├── SnapshotService        # 代码快照备份/回滚（29.8KB）
...
| **工具系统** | 30+ 工具注册/执行/权限/后处理 | tool/ 整个包 | ~6000 |
| **用户权限** | RBAC、Token 认证、菜单控制 | UserServiceImpl + Filter | ~500 |
| **定时任务** | Cron/一次性调度、执行追溯 | ScheduleTaskScheduler | ~350 |
...
4. **密码加密**: MD5（32位），不是 bcrypt——安全敏感场景需升级
