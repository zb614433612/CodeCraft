    ...
    <header class="page-header">
      <div class="header-left">
        ...
        <div class="header-text">
          <h2 class="page-title">LLM Provider 管理</h2>
    ...
    <div class="agent-content">
      ...
      <div v-else-if="providerList.length === 0" class="state-wrapper">
        <div class="state-card empty-card">
          ...
          <p class="empty-desc">配置一个 LLM 平台（DeepSeek / OpenAI / Claude 等）以启用 AI 助手</p>
            ...
            <div class="modal-body">
              <a-form ref="formRef" :model="formData" :label-col="{ span: 5 }" :wrapper-col="{ span: 19 }" class="agent-form">
                ...
                <a-form-item label="API Key">
                  <!-- 已配置 + 未展开修改：显示指示器 -->
                  <div v-if="isEditing && apiKeyConfigured && !showApiKeyInput" class="apikey-configured">
                    <span class="apikey-dot"></span>
                    <span class="apikey-label">API Key 已配置</span>
                    <a-button type="link" size="small" class="apikey-change-btn" @click="showApiKeyInput = true; formData.apiKey = ''">
                      修改 Key
                    </a-button>
                  </div>
                  <!-- 正常输入 / 新建 / 展开修改 -->
                  <a-input-password
                    ...
                    :placeholder="apiKeyConfigured ? '输入新 Key 替换旧 Key' : '输入 API Key'"
                    :maxLength="200"
                    size="large"
                    class="form-input"
                  />
...
const apiKeyConfigured = ref(false)      // 当前编辑的 Provider 是否已配置 Key
const showApiKeyInput = ref(false)       // 是否显示 Key 修改输入框
...
function templateStyle(template: string): Record<string, string> {
  ...
  return {
    ...
    fontWeight: '700', padding: '3px 10px', borderRadius: '10px',
...
async function handleSave() {
  ...
  try {
    ...
    // 编辑模式下，如果 Key 已配置但用户未修改，发送掩码告知后端保持原值
    if (isEditing.value && apiKeyConfigured.value && !showApiKeyInput.value) {
      payload.apiKey = '****'
    }
...
.modal-container { --modal-bg: #ffffff; --modal-text-1: #1a1a2e; --modal-text-2: #5c5c78; --modal-text-3: #9696aa; --modal-border: #e8e5f0; background: var(--modal-bg); border-radius: 18px; width: 680px; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 20px 60px rgba(0,0,0,0.2); color: var(--modal-text-1); }
.modal-header { display: flex; align-items: flex-start; gap: 14px; padding: 20px 24px 16px; border-bottom: 1px solid var(--modal-border); border-radius: 18px 18px 0 0; flex-shrink: 0; }
...
.modal-close { background: none; border: none; font-size: 20px; cursor: pointer; color: var(--modal-text-3); padding: 4px 10px; border-radius: 8px; transition: all 0.2s; flex-shrink: 0; line-height: 1; }
.modal-close:hover { color: var(--modal-text-1); background: #f0edf6; }
.modal-body { flex: 1; overflow-y: auto; padding: 20px 24px; }
.modal-body::-webkit-scrollbar { width: 5px; } .modal-body::-webkit-scrollbar-thumb { background: #dcd8ea; border-radius: 3px; }
.modal-footer { display: flex; justify-content: flex-end; gap: 12px; padding: 16px 24px; border-top: 1px solid var(--modal-border); background: #faf9fc; border-radius: 0 0 18px 18px; flex-shrink: 0; }
...
[data-theme="dark"] .provider-config-root { --bg-root: #121117; --bg-card: #1a1925; --bg-hover: rgba(139,92,246,0.06); --text-1: #e4e2f0; --text-2: #a09eb8; --text-3: #6a6880; --text-4: #525070; --border: #2a2838; --border-lt: #222030; --shadow-sm: 0 1px 3px rgba(0,0,0,0.3); --shadow-md: 0 4px 12px rgba(0,0,0,0.4); --shadow-lg: 0 8px 30px rgba(0,0,0,0.5); --accent-lt: rgba(139,92,246,0.1); --accent-md: rgba(139,92,246,0.2); --accent-glow: rgba(139,92,246,0.2); }
...
[data-theme="dark"] .modal-container { --modal-bg: #1e1d2c; --modal-text-1: #e4e2f0; --modal-text-2: #a09eb8; --modal-text-3: #7a7898; --modal-border: #363448; box-shadow: 0 20px 60px rgba(0,0,0,0.5); }
...
.apikey-label { font-weight: 600; color: #059669; }
.apikey-change-btn { padding: 0 4px !important; font-size: 12px !important; height: auto !important; color: var(--accent, #8b5cf6) !important; }
.apikey-change-btn:hover { color: var(--accent-dk, #7c3aed) !important; text-decoration: underline; }
