> 🌐 English Version：[🇬🇧 BUILD_AND_RUN_EN](./BUILD_AND_RUN_EN.md)
# CodeCraft 构建与运行指南
...
CodeCraft 需要配置至少一个 LLM Provider 才能正常使用。支持 DeepSeek、OpenAI、Anthropic、Ollama、MiMo、MiniMax 等多种大语言模型。
...
>
> ⚠️ 注意：API Key **不通过** `application-local.yml` 配置——请在页面「Provider 配置」中添加 LLM Provider（支持 DeepSeek / OpenAI / Anthropic / Ollama / MiMo / MiniMax 等多种模型）。
...
# 输出文件
# target/code-craft-1.1.7.jar  （约 67MB）
```
...
```
electron/release/
├── CodeCraft-Setup-1.1.7.exe       # NSIS 安装程序（发行用）
└── CodeCraft-1.1.7-win.zip         # 绿色版压缩包（可选）
```

### 安装运行

双击 `CodeCraft-Setup-1.1.7.exe` 安装，安装后桌面会生成快捷方式。
...
```
electron/release/
├── CodeCraft-1.1.7.dmg              # DMG 磁盘映像（发行用）
├── CodeCraft-1.1.7-mac.zip          # 绿色版压缩包（可选）
└── mac/                             # 未打包的 .app 目录
    └── CodeCraft.app
```
...
1. 双击 `CodeCraft-1.1.7.dmg` 挂载磁盘映像
...
```
electron/release/
├── CodeCraft-1.1.7.AppImage    # AppImage（免安装，双击运行）
└── CodeCraft-1.1.7.deb         # deb 包（Debian/Ubuntu）
```
...
# 4. 安装
# 双击 electron/release/CodeCraft-Setup-1.1.7.exe
```
...
# 5. 安装
# 双击 electron/release/CodeCraft-1.1.7.dmg 挂载后拖入 Applications
```
