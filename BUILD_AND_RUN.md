...
# 输出文件
# target/codecraft-1.0.2.jar  （约 67MB）
```
...
```
electron/release/
├── CodeCraft-Setup-1.0.2.exe       # NSIS 安装程序（发行用）
└── CodeCraft-1.0.2-win.zip         # 绿色版压缩包（可选）
```

### 安装运行

双击 `CodeCraft-Setup-1.0.2.exe` 安装，安装后桌面会生成快捷方式。
...
```
electron/release/
├── CodeCraft-1.0.2.dmg              # DMG 磁盘映像（发行用）
├── CodeCraft-1.0.2-mac.zip          # 绿色版压缩包（可选）
└── mac/                             # 未打包的 .app 目录
    └── CodeCraft.app
```
...
1. 双击 `CodeCraft-1.0.2.dmg` 挂载磁盘映像
...
```
electron/release/
├── CodeCraft-1.0.2.AppImage    # AppImage（免安装，双击运行）
└── CodeCraft-1.0.2.deb         # deb 包（Debian/Ubuntu）
```
...
# 4. 安装
# 双击 electron/release/CodeCraft-Setup-1.0.2.exe
```
...
# 5. 安装
# 双击 electron/release/CodeCraft-1.0.2.dmg 挂载后拖入 Applications
```
